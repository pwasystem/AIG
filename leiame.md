...
2. **Requisito essencial**: Instale o [Ollama](https://ollama.com/) na sua máquina. O sistema exige a instalação dos seguintes modelos locais para funcionar:
   * **qwen2.5:3b** (Agente Estudante) -> Comando: `ollama run qwen2.5:3b`
   * **llama3:8b** (Agente Professor) -> Comando: `ollama run llama3:8b`
3. Opcional: Execute o `install.bat` caso precise instalar as dependências.
4. Para abrir o sistema e iniciar a interface web, basta rodar o comando:
   ```bash
   .\run_web.bat
   ```
